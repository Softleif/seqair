...
#[hegel::test]
fn depth_matches_sweep_line_count(tc: TestCase) {
    let records = tc.draw(
        gs::vecs(gs::tuples!(
            gs::integers::<i32>().min_value(0).max_value(499),
            gs::integers::<u32>().min_value(10).max_value(100),
        ))
        .min_size(1)
        .max_size(20),
    );
    ...
    let mut engine = PileupEngine::new(
        ...
        (Pos0::new(0).unwrap()..=Pos0::new(600).unwrap()).into(),
    );
    while let Some(col) = engine.pileups() {
        let pos = col.pos().as_usize();
        let exp = expected_depth.get(pos).copied().unwrap_or(0);
        assert_eq!(col.depth(), exp, "depth mismatch at pos {}", col.pos());
    }
}
...
#[hegel::test]
fn columns_match_sweep_line_coverage(tc: TestCase) {
    let records = tc.draw(
        gs::vecs(gs::tuples!(
            gs::integers::<i32>().min_value(0).max_value(499),
            gs::integers::<u32>().min_value(10).max_value(100),
    ...
    for pos in 0..MAX_POS as u32 {
        ...
        assert_eq!(
            actual_covered, exp_covered,
            "position {} coverage mismatch (sweep says covered={})",
            pos, exp_covered
        );
    }
}
...
fn keep_record_by_flags_excludes_correct_reads(tc: TestCase) {
    ...
    let mut engine = PileupEngine::new(
        arena.prepare_for_pileup().input,
        (Pos0::new(0).unwrap()..=Pos0::new(0).unwrap()).into(),
    );
    let columns = collect_columns(&mut engine);

    if expected > 0 {
        assert_eq!(columns[0].depth(), expected);
    } else {
...
#[hegel::test]
fn max_depth_never_exceeded(tc: TestCase) {
    let max = tc.draw(gs::integers::<u32>().min_value(1).max_value(10));
    let n = tc.draw(gs::integers::<usize>().min_value(1).max_value(30));
    let len = tc.draw(gs::integers::<u32>().min_value(10).max_value(50));
    let mut arena = RecordStore::new();
    for _ in 0..n {
    ...
    while let Some(col) = engine.pileups() {
        assert!(col.depth() <= max as usize);
    }
}

// r[verify pileup.max_depth_per_position]
#[hegel::test]
fn reads_available_at_lower_coverage_despite_cap(tc: TestCase) {
    let extra = tc.draw(gs::integers::<usize>().min_value(3).max_value(10));
    let mut arena = RecordStore::new();
    for _ in 0..2 {
...
#[hegel::test]
fn qpos_for_simple_cigar_is_offset_from_start(tc: TestCase) {
    let start = tc.draw(gs::integers::<i32>().min_value(0).max_value(99));
    let len = tc.draw(gs::integers::<u32>().min_value(10).max_value(50));
    let mut arena = RecordStore::new();
    ...
    let mut engine = PileupEngine::new(
        arena.prepare_for_pileup().input,
        (Pos0::new(start_u32).unwrap()..=Pos0::new(end_u32).unwrap()).into(),
    );

    while let Some(col) = engine.pileups() {
        for aln in col.alignments() {
            assert_eq!(aln.qpos().unwrap(), QPos::new(col.pos().as_u32() - start_u32));
        }
    }
}
...
#[hegel::test]
fn no_empty_columns(tc: TestCase) {
    let pos1 = tc.draw(gs::integers::<i32>().min_value(0).max_value(99));
    let len1 = tc.draw(gs::integers::<u32>().min_value(5).max_value(20));
    let gap = tc.draw(gs::integers::<i32>().min_value(50).max_value(500));
    let len2 = tc.draw(gs::integers::<u32>().min_value(5).max_value(20));
    ...
    let mut engine = PileupEngine::new(
        arena.prepare_for_pileup().input,
        (Pos0::new(pos1 as u32).unwrap()..=Pos0::new(end).unwrap()).into(),
    );
    while let Some(col) = engine.pileups() {
        assert!(col.depth() > 0, "empty column at {}", col.pos());
    }
}
...
fn unmapped_reads_excluded_from_pileup() {
    ...
    let mut engine = PileupEngine::new(
        arena.prepare_for_pileup().input,
        (Pos0::new(100).unwrap()..=Pos0::new(149).unwrap()).into(),
    );
    let columns = collect_columns(&mut engine);
...
fn zero_refspan_read_handled_gracefully() {
    ...
    assert_eq!(arena.record(ri(0)).unwrap().end_pos, Pos0::new(50).unwrap());

    let mut engine = PileupEngine::new(
        arena.prepare_for_pileup().input,
        (Pos0::new(48).unwrap()..=Pos0::new(52).unwrap()).into(),
    );
    let columns = collect_columns(&mut engine);
...
fn leading_softclip_does_not_extend_ref_range() {
    ...
    let mut engine = PileupEngine::new(
        arena.prepare_for_pileup().input,
        (Pos0::new(95).unwrap()..=Pos0::new(125).unwrap()).into(),
    );
    let columns = collect_columns(&mut engine);
    ...
    assert_eq!(columns[0].alignments().next().unwrap().qpos().unwrap(), QPos::new(5));
}
...
// r[verify pileup.position_iteration]
#[hegel::test]
fn column_positions_are_strictly_increasing(tc: TestCase) {
    let records = tc.draw(
        gs::vecs(gs::tuples!(
            gs::integers::<i32>().min_value(0).max_value(199),
            gs::integers::<u32>().min_value(10).max_value(50),
        ))
        .min_size(1)
        .max_size(15),
    );
    ...
    while let Some(col) = engine.pileups() {
        ...
        if let Some(prev) = prev_pos {
            assert!(cur > prev, "positions must be strictly increasing: {} not > {}", cur, prev);
        }
        prev_pos = Some(cur);
    }
}
...
// r[verify pileup.qpos]
#[hegel::test]
fn qpos_within_read_length(tc: TestCase) {
    let start = tc.draw(gs::integers::<i32>().min_value(0).max_value(99));
    let len = tc.draw(gs::integers::<u32>().min_value(10).max_value(100));
    let mut arena = RecordStore::new();
    ...
    while let Some(col) = engine.pileups() {
        for aln in col.alignments() {
            if let Some(qpos) = aln.qpos() {
                assert!(
                    qpos.as_usize() < len as usize,
                    "qpos {} must be < seq_len {} at pos {}",
                    qpos,
                    len,
                    col.pos()
                );
...
// r[verify pileup.column_contents]
#[hegel::test]
fn all_alignments_have_valid_base_and_qual(tc: TestCase) {
        ...
        for aln in col.alignments() {
            ...
            if let (Some(base), Some(qual)) = (aln.base(), aln.qual()) {
                ...
                assert!(
                    ...
                    "invalid base byte {} at pos {}",
                    base_byte,
                    col.pos()
                );
                // Qual was set to 30 in make_record
                assert_eq!(qual.get(), Some(30), "qual should be 30");
            }
        }
    }
}
...
#[hegel::test]
fn trailing_empty_positions_terminate_early(tc: TestCase) {
    let start = tc.draw(gs::integers::<i32>().min_value(0).max_value(99));
    let len = tc.draw(gs::integers::<u32>().min_value(10).max_value(30));
    let trailing = tc.draw(gs::integers::<u32>().min_value(100).max_value(500));
    let mut arena = RecordStore::new();
    arena.push_raw(&make_record(0, start, 99, 60, len), &mut ()).unwrap();
    ...
    let mut engine = PileupEngine::new(
        arena.prepare_for_pileup().input,
        (Pos0::new(start_u32).unwrap()..=Pos0::new(region_end).unwrap()).into(),
    );
    let columns = collect_columns(&mut engine);
    ...
    assert_eq!(
        columns.len(),
        len as usize,
        "should terminate after last record, not iterate {} trailing positions",
        trailing
    );
}
...
fn reference_base_unknown_without_ref_seq() {
    ...
    let mut engine = PileupEngine::new(
        arena.prepare_for_pileup().input,
        (Pos0::new(0).unwrap()..=Pos0::new(4).unwrap()).into(),
    );
    while let Some(col) = engine.pileups() {
...
fn reclaim_allocation_reuse_across_regions() {
    ...
    let mut engine = PileupEngine::new(
        arena.prepare_for_pileup().input,
        (Pos0::new(0).unwrap()..=Pos0::new(9).unwrap()).into(),
    );
    ...
    let mut engine2 = PileupEngine::new(
        ...
        (Pos0::new(100).unwrap()..=Pos0::new(104).unwrap()).into(),
    );
...
// r[verify pileup.active_set]
#[hegel::test]
fn read_starting_before_region_contributes_within(tc: TestCase) {
    let read_start = tc.draw(gs::integers::<i32>().min_value(0).max_value(49));
    let read_len = tc.draw(gs::integers::<u32>().min_value(60).max_value(100));
    let region_start_offset = tc.draw(gs::integers::<u32>().min_value(10).max_value(40));
    let mut arena = RecordStore::new();
    ...
    for col in &columns {
        assert!(col.pos() >= Pos0::new(region_start).unwrap(), "no column before region_start");
        assert!(col.pos() <= Pos0::new(region_end).unwrap(), "no column after region_end");
    }
}
...
// r[verify pileup.unmapped_excluded]
#[hegel::test]
fn unmapped_reads_never_appear_in_columns(tc: TestCase) {
    let n_mapped = tc.draw(gs::integers::<usize>().min_value(1).max_value(8));
    let n_unmapped = tc.draw(gs::integers::<usize>().min_value(1).max_value(5));
    let mut arena = RecordStore::new();
    for _ in 0..n_mapped {
        arena.push_raw(&make_record(0, 0, 99, 60, 20), &mut ()).unwrap();
    }
    ...
    while let Some(col) = engine.pileups() {
        assert_eq!(
            col.depth(),
            n_mapped,
            "unmapped reads should be excluded, expected {} at pos {}",
            n_mapped,
            col.pos()
        );
    }
...
fn depth_matches_cigar_derived_coverage(tc: TestCase) {
    ...
    let region_end = reads.iter().map(|r| r.pos as u32 + r.ref_span + 10).max().unwrap();
    ...
    while let Some(col) = engine.pileups() {
        ...
        let expected_match_depth = reads
            .iter()
            .filter(|r| r.covered_ref_positions().contains(&col.pos().as_i64()))
            .count();
        assert_eq!(
            match_depth,
            expected_match_depth,
            "match-depth mismatch at pos {} (expected from CIGAR analysis)",
            col.pos()
        );
    }
}
...
fn qpos_matches_independent_cigar_walk(tc: TestCase) {
    ...
    while let Some(col) = engine.pileups() {
        for aln in col.alignments() {
            ...
            assert_eq!(
                aln.qpos(),
                expected_qpos.map(|q| QPos::new(q as u32)),
...
fn deletions_cause_absent_alignment(tc: TestCase) {
    ...
    while let Some(col) = engine.pileups() {
        ...
        assert_eq!(
            col.depth(),
            1,
            "position {} should have depth 1 (all ref-consuming ops contribute)",
            col.pos()
        );
        let aln = col.alignments().next().unwrap();
        if covered.contains(&col.pos().as_i64()) {
            assert!(aln.qpos().is_some(), "position {} is M/=/X, should have qpos", col.pos());
        } else {
            assert!(
                aln.is_del() || aln.is_refskip(),
                "position {} is D/N, should be Deletion or RefSkip, got {:?}",
                col.pos(),
                aln.op
            );
            assert!(aln.qpos().is_none(), "position {} is D/N, should have no qpos", col.pos());
        }
    }
}
...
fn insertions_shift_qpos_but_not_depth(tc: TestCase) {
    ...
    while let Some(col) = engine.pileups() {
        ...
        if let Some(expected) = expected_qpos {
            ...
            assert_eq!(
                aln.qpos(),
                Some(QPos::new(expected as u32)),
                "qpos wrong at pos {} — insertion should shift query offset",
                col.pos()
            );
        }
    }
...
fn no_zero_depth_columns_with_complex_cigars(tc: TestCase) {
    ...
    let region_start = reads.iter().map(|r| r.pos as u32).min().unwrap();
    let region_end = reads.iter().map(|r| r.pos as u32 + r.ref_span + 100).max().unwrap();
    ...
    while let Some(col) = engine.pileups() {
        assert!(col.depth() > 0, "empty column at pos {} with complex CIGARs", col.pos());
    }
}
...
fn qpos_always_within_seq_bounds(tc: TestCase) {
    ...
    while let Some(col) = engine.pileups() {
        for aln in col.alignments() {
            if let Some(qpos) = aln.qpos() {
                assert!(
                    qpos.as_usize() < aln.seq_len as usize,
...
fn pileup_includes_out_of_order_records() {
    ...
    let mut engine = PileupEngine::new(
        store.prepare_for_pileup().input,
        (Pos0::new(80).unwrap()..=Pos0::new(280).unwrap()).into(),
    );
    let columns = collect_columns(&mut engine);

    // At positions 100..150, all three reads should be active:
    // r1 (100..150), r3 (80..280), and optionally r2 doesn't reach here
    let col_at_120 = columns.iter().find(|c| c.pos().as_u32() == 120).expect("column at 120");
    assert_eq!(
        col_at_120.depth(),
        2,
    ...
    let col_at_220 = columns.iter().find(|c| c.pos().as_u32() == 220).expect("column at 220");
    assert_eq!(
        col_at_220.depth(),
        2,
    ...
    let col_at_90 = columns.iter().find(|c| c.pos().as_u32() == 90).expect("column at 90");
...
fn pileup_deduplicates_cross_category_records() {
    ...
    let mut engine = PileupEngine::new(
        store.prepare_for_pileup().input,
        (Pos0::new(100).unwrap()..=Pos0::new(170).unwrap()).into(),
    );
    let columns = collect_columns(&mut engine);

    // At position 120, both r1 and r2 cover it, but r1 should appear only once
    let col = columns.iter().find(|c| c.pos().as_u32() == 130).expect("column at 130");
