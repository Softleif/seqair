...
fn deletion_positions_have_deletion_op() {
    ...
    let engine = PileupEngine::new(arena, 100, 124);
    let columns: Vec<_> = engine.collect();

    // Positions 100-109: Match
    for col in columns.iter().filter(|c| c.pos() >= 100 && c.pos() <= 109) {
    ...
    for col in columns.iter().filter(|c| c.pos() >= 110 && c.pos() <= 114) {
        let aln = col.alignments().next().unwrap();
    ...
    for col in columns.iter().filter(|c| c.pos() >= 115 && c.pos() <= 124) {
        let aln = col.alignments().next().unwrap();
        assert!(
            matches!(aln.op, PileupOp::Match { .. }),
            "pos {} should be Match, got {:?}",
...
fn refskip_positions_have_refskip_op() {
    ...
    let engine = PileupEngine::new(arena, 0, 119);
    let columns: Vec<_> = engine.collect();
...
fn depth_counts_deletions_and_refskips() {
    ...
    let col7 = columns.iter().find(|c| c.pos() == 7).unwrap();
...
fn insertion_reported_at_last_match_before_insert() {
    ...
    let col9 = columns.iter().find(|c| c.pos() == 9).unwrap();
    ...
    let col8 = columns.iter().find(|c| c.pos() == 8).unwrap();
    ...
    let col10 = columns.iter().find(|c| c.pos() == 10).unwrap();
...
fn insertion_after_deletion_not_reported() {
    ...
    for pos in 10..15 {
        let col = columns.iter().find(|c| c.pos() == pos).unwrap();
        let aln = col.alignments().next().unwrap();
...
fn insertion_before_deletion() {
    ...
    let col15 = columns.iter().find(|c| c.pos() == 15).unwrap();
...
fn minimal_insertion_1m_1i_1m() {
    ...
    let engine = PileupEngine::new(arena, 0, 1);
    let columns: Vec<_> = engine.collect();
    ...
    let col1 = columns.iter().find(|c| c.pos() == 1).unwrap();
...
proptest! {
    ...
    fn every_ref_position_produces_column_for_single_read(read in arb_read()) {
        ...
        let region_end = region_start + i64::from(read.ref_span) - 1;
