...
mod e2e_oracle {
    ...
    #[hegel::composite]
    fn arb_e2e_input(tc: &TestCase) -> E2eInput {
        let qname = tc.draw_silent(
            gs::vecs(gs::integers::<u8>().min_value(b'a').max_value(b'z')).min_size(1).max_size(12),
        );
        let bases = tc.draw_silent(gs::vecs(gs::sampled_from(&ACGT)).min_size(1).max_size(12));
        let pos = tc.draw_silent(gs::integers::<u32>().max_value(49_999));
    ...
    #[hegel::test]
    fn push_fields_then_bam_write_then_read_round_trips(tc: TestCase) {
        let mut inputs =
            tc.draw(gs::vecs(arb_e2e_input().print_as_debug()).min_size(1).max_size(20));
        ...
        for (i, inp) in inputs.iter().enumerate() {
            ...
            assert_eq!(rec.pos.as_i32() as u32, inp.pos, "pos rec {}", i);
            assert_eq!(rec.mapq, inp.mapq, "mapq rec {}", i);
            assert_eq!(rec.seq_len as usize, inp.bases.len(), "seq_len rec {}", i);
            assert_eq!(
                store2.record(idx).unwrap().qname(),
                inp.qname.as_slice(),
                "qname rec {}",
                i
            );
            assert_eq!(store2.record(idx).unwrap().seq(), inp.bases.as_slice(), "seq rec {}", i);
            let quals_back = BaseQuality::slice_to_bytes(store2.record(idx).unwrap().qual());
            assert_eq!(quals_back, inp.quals.as_slice(), "qual rec {}", i);
            let cigar = store2.record(idx).unwrap().cigar();
            assert_eq!(cigar.len(), 1, "cigar op count rec {}", i);
            assert_eq!(cigar[0].op_type(), CigarOpType::Match, "cigar op type rec {}", i);
            assert_eq!(cigar[0].len() as usize, inp.bases.len(), "cigar op len rec {}", i);
        }
    }
}
