...
> **Sources:** Seqair-specific design. Inspired by [htslib] `bam_pileup1_t` which provides `is_del`, `is_refskip`, and `indel` fields, but redesigned as a Rust enum for compile-time safety. CIGAR operation semantics from [SAM1] §1.4. See [References](./99-references.md).

## PileupOp enum

> _[SAM1] §1.4 — CIGAR operations: M/=/X consume ref+query, D consumes ref only, N is ref-skip, I consumes query only_
