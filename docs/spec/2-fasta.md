...
A byte offset that is not addressable in `u64` MUST likewise produce an error naming the sequence, not a panic — and not a wrapped value that is silently treated as valid. `byte_offset` MUST use checked arithmetic and return `None` on overflow: `offset` comes from the untrusted index and can sit near `u64::MAX`, and wrapping arithmetic there is not just a panic risk. Because both endpoints of a fetch's span share the same `offset`, a naive wraparound shifts each endpoint down by the same amount, so the computed span *length* still comes out correct — the reader would silently serve bytes from the wrong location in the file rather than erroring or producing an obviously-wrong (negative-length) span.
