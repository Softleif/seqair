...
> _[CSI] — CSI format table: magic `CSI\x01`, min_shift, depth, l_aux, n_ref; per-bin loffset replacing linear index; parameterised reg2bin/reg2bins/bin_limit functions_

r[csi.magic]
CSI files MUST start with magic bytes `CSI\x01` (4 bytes), after BGZF decompression.

> r[csi.header]
> CSI files MUST continue with these fiels after magic bytes:
>
