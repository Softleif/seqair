...
> _[BCF2] — magic `BCF\2\1`, l_text header length, NUL-terminated VCF header text_
...
> _[BCF2] — record layout: l_shared, l_indiv, shared data (site), individual data (samples)_
...
> _[BCF2] — typed value encoding: type byte (count << 4 | type_code), type codes 1=int8, 2=int16, 3=int32, 5=float, 7=char_
...
> _[BCF2] — missing and end-of-vector sentinel values table. See also [hts-specs issue #145](https://github.com/samtools/hts-specs/issues/145) on signaling NaN handling_
...
> _[BCF2] — FORMAT/sample encoding: field-major layout, key + type + n_sample values per FORMAT field_
...
> _[BCF2] — GT encoding: `(allele+1) << 1 | phased`, missing allele = 0x00_
