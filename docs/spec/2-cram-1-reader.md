...
The header is untrusted: the name count MUST be at most 10 million (`r[cram.tok3.name_count_limit]`), the uncompressed length MUST pass the codec output cap (`r[io.fuzz.codec_output_cap]`), and the output MUST NOT exceed the uncompressed length (htscodecs allows about 1 KiB more; its encoder writes the exact length) — so a name count above the uncompressed length, which leaves less than a NUL per name, is rejected before anything is allocated for the names.
...
r[cram.codec.simd_dispatch+3]
rANS Nx16 decoding takes a packed fast path whenever the frequency table allows it: each table slot is one `u32` holding the symbol, its frequency minus one, and the slot's offset into the symbol's range (htscodecs' layout), so a decode step is one load. A table packs only if its frequencies sum to exactly `1 << bits` (for order-1: every active context's row, and context 0 is active); every slot then belongs to a symbol with a non-zero frequency, the packed step equals `state_step` bit for bit, and a step leaves any state ≥ `1 << 15` at ≥ 8, so one 16-bit renormalization read suffices. The 32-state decoders (order-0 and order-1) are SIMD kernels (`r[io.simd_portable]`): scalar table loads, then the state step and the renormalization on 4-lane vectors — lanes below `1 << 15` take the next u16s of the stream in lane order through a shuffle chosen by the lane mask, the stream advancing by twice the mask's popcount. The kernel runs only while every initial state was ≥ `1 << 15` and at least `2·N + 16` bytes remain; the rest of the block runs the same packed step with the scalar, bounds-checked renormalization. The 4-state decoders run the packed step with a branch-free scalar renormalization. Tables that do not pack take the unpacked scalar decoders, which stay the reference: every level's output, consumed length and error-or-not MUST equal theirs. Errors propagate — there is no snapshot/restore and scalar retry, which would hide kernel bugs from that comparison.

r[cram.codec.rans_nx16_o1_tile]
The 32-state order-1 SIMD kernel writes each step's 32 symbols as one row of a 32×32 tile, which also holds each state's context (the previous row), and every 32 steps transposes the tile into 32-byte runs of the states' output stripes, 16×16 quarters at a time through byte interleaves (htscodecs' `transpose_and_copy`), instead of 32 single-byte stores to 32 stripes a step. The rows of a last, partial tile are written a byte at a time before the scalar steps continue. Output MUST equal the unpacked decoder's.

r[cram.codec.order1_table_reset]
The order-1 frequency tables are reused across blocks. Reading a block's table MUST zero each active context's row before filling it (a stream skips zero-frequency symbols rather than writing them), and the unpacked decoder MUST zero the rows of inactive contexts, so a block decodes exactly as it would with a fresh buffer whatever an earlier block left behind.

r[cram.codec.rans4x8_packed+2]
rANS 4x8 order-0 decoding keeps htscodecs' split slot tables — per slot the symbol (`u8`), its frequency and the slot's offset into the symbol's range (`u16` each) — built from the reference decoder's slot scan, so a step is `freq · (x >> 12) + offset` and equals the reference step bit for bit whatever the total. The fast loop renormalizes in htscodecs' `RansDecRenorm` shape: per round of four symbols one 8-byte window, per state the first byte taken branch-free (a select and a carry into the window cursor) and the rare second byte behind a branch, with no variable shifts and no per-state bounds checks. Two bytes suffice when a slot's frequency is 1 to 4096, which leaves a stepped state ≥ 2^11. A table whose total is under 4096 with a zero frequency for symbol 255 has slots (from the total on) that break this — htscodecs' encoder normalizes to 4095, so slot 4095 is one — and the fast loop stops before a round that meets one; a total over 4096 runs no fast rounds. The loop runs while every initial state was ≥ 2^23 and 8 bytes remain; the rest runs checked. Output, consumed length and error-or-not MUST equal the reference decoder's.

r[cram.codec.rans4x8_fast+2]
rANS 4x8 order-1 decoding keeps htscodecs' split tables — per context a slot → symbol table (`u8`, 4 KiB) and a symbol → `(freq, cum)` table (1 KiB) — rather than 16 KiB packed slots, so a quality stream's few dozen contexts stay cache-resident. The step is the reference decoder's arithmetic on those values, so every table decodes as there, whatever its total. A block rebuilds only the rows it can reach: its active contexts, and — from all-zero frequencies, as the reference decoder does for every inactive context — context 0, 255 and each symbol an active context can decode. The fast loop renormalizes as the order-0 one does (`r[cram.codec.rans4x8_packed+2]`: one 8-byte window per round, the first byte a state takes branch-free, the rare second behind a branch); that is exact when a step leaves the state ≥ 2^11, and a round where some state falls below runs checked. It runs while every initial state was ≥ 2^23 and 8 bytes remain; the rest runs checked. Output, consumed length and error-or-not MUST equal the reference decoder's.
