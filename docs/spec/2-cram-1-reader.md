...
r[cram.codec.rans4x8_fast]
rANS 4x8 order-1 decoding keeps htscodecs' split tables — per context a slot → symbol table (`u8`, 4 KiB) and a symbol → `(freq, cum)` table (1 KiB) — rather than 16 KiB packed slots, so a quality stream's few dozen contexts stay cache-resident. The step is the reference decoder's arithmetic on those values, so every table decodes as there, whatever its total. A block rebuilds only the rows it can reach: its active contexts, and — from all-zero frequencies, as the reference decoder does for every inactive context — context 0, 255 and each symbol an active context can decode. The fast loop renormalizes all four states from one 8-byte big-endian load, taking 0, 1 or 2 bytes a state; that is exact when a step leaves the state ≥ 2^11, and a round where some state falls below runs checked. It runs while every initial state was ≥ 2^23 and 8 bytes remain; the rest runs checked. Output, consumed length and error-or-not MUST equal the reference decoder's.
