...
rANS 4x8 decoding takes the packed tables of `r[cram.codec.simd_dispatch+3]` with a branch-free byte renormalization (0, 1 or 2 bytes, big-endian, in one shift) whenever every active context's frequencies sum to 4096 or to 4095 — htscodecs' 4x8 encoder normalizes to 4095. In a 4095 table slot 4095 belongs to no symbol; the unpacked decoder's scan gives it symbol 255 with 255's frequency, possibly zero, which a packed slot cannot hold, so that slot is a sentinel the decoder computes exactly. Each 4x8 context has its own symbol set, so a decoded symbol need not be an active context: every reachable inactive context (0, any symbol with a frequency, and 255 behind a sentinel or an inactive row) gets the unpacked decoder's all-zero row, as sentinels. The fast loop runs while every initial state was ≥ 2^23 and 8 bytes remain, and stops before a step that meets a sentinel; the rest runs checked. Output, consumed length and error-or-not MUST equal the unpacked decoders', which stay for tables that do not pack.
