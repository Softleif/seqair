...
r[cram.codec.simd_dispatch+2]
The 32-state order-0 rANS Nx16 decode is one SIMD kernel (`r[io.simd_portable]`): the state step runs on native-width vectors of states, the symbol lookup and renormalization stay scalar and shared with the scalar decoder. Errors from the kernel MUST propagate to the caller — there is no snapshot/restore and scalar retry. The kernel and the scalar decoder run the same algorithm over the same `src` and `states`, so the only legitimate way for the kernel to fail (truncation) is also a way for the scalar decoder to fail; a retry would hide kernel bugs from the property test that compares every level against `decode_order_0_generic`.
